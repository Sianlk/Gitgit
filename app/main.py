"""
GitGit — FastAPI Backend
Domain: devops
AI Focus: commit analysis, PR generation, code review, branch strategy
Features: AI Commits, Smart PR Generator, Code Review AI, Branch Strategy, Release Notes AI
"""
...
async def root():
    return {
        "service": "GitGit",
        "tagline": "AI-powered git workflow automation",
        "domain": "devops",
        "version": "1.0.0",
        "docs": "/docs",
        "status": "live",
...
Focus area: commit analysis, PR generation, code review, branch strategy.
Tagline: AI-powered git workflow automation.
Provide accurate, helpful, domain-specific responses."""
    ...
    if not openai_key:
        ...
        content = f"[DEV MODE] GitGit AI received: {request.prompt[:100]}"
...
async def run_agent(task: AgentTaskRequest, req: Request):
    ...
    AI_REQUESTS.labels("gpt-4o", "devops").inc()
    return {
...
async def websocket_ai(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            await websocket.send_text(f"[GitGit AI] Processing: {data[:50]}")
            await asyncio.sleep(0.1)
            await websocket.send_text(f"[STREAM] Domain: devops | Response ready")
